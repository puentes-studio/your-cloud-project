...
const FoldersAndFiles = ({ carpeta, files, setFiles }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        let response;
        if (carpeta) {
          response = await getFilesInFolder(authState.dataLogin.token, carpeta);
        } else {
          response = await getFoldersAndFiles(authState.dataLogin.token);
        }
  ...
  }, [authState.dataLogin.token, carpeta, setFiles]); // Dependencia de efecto: token
  ...
  const handleDeleteFile = async (fileId) => {
    ...
    try {
      await deleteFile(authState.dataLogin.token, fileId);
          ...
          <div className="file-grid">
            ...
            {files.map((file) => (
              <div key={file.id} className="file-item">
                ...
                <a
                  className="file-name"
                  href={`${import.meta.env.VITE_URL_API}/${file.user_id}${
                    !carpeta ? '' : '/' + carpeta
                  }/${file.file_name}`}
                  target={'_blank'}
                  alt={file.file_name}
